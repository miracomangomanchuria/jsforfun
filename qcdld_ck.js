...
2) qcdld简版主页面，触发请求即可写入。

【MITM】
在 QX 的 MITM 中添加：dld.qzapp.z.qq.com

【保存字段】
ptcz、openId、accessToken、newuin
...
!(async () => {
  ...
  const ptcz = matchCookie(cookie, "ptcz");
  const openId = matchCookie(cookie, "openId");
  const accessToken = matchCookie(cookie, "accessToken");
  const newuin = matchCookie(cookie, "newuin");

  if (!ptcz || !openId || !accessToken || !newuin) {
    $.notify(
      "qcdld_Cookie",
      "字段不完整",
      "缺少: " +
